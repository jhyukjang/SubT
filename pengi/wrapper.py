...
class PengiWrapper():
    ...
    def filter_audio(self, a, sr, cutoff, order=5):
        audio_np = a.numpy().squeeze()
        ...
        low_filtered_np = lfilter(b_low, a_low, audio_np)
        high_filtered_np = lfilter(b_high, a_high, audio_np)

        low_filtered = torch.from_numpy(low_filtered_np).unsqueeze(0).float()
        high_filtered = torch.from_numpy(high_filtered_np).unsqueeze(0).float()
